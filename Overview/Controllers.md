...
| Standard(추천) | 이 내장 메소드를 사용하면, 요청 핸들러가 JavaScript 객체나 배열을 반환할 때 자동으로 JSON으로 변환해준다. 그러나 핸들러가 JavaScript 원시 타입을 반환한다면(예를 들어, string, number, boolean) Nest는 JSON으로 변환하지 않는다. 이는 응답 핸들링을 쉽게 만들어준다.

| 더 나아가, 응답 상태 코드는 201을 사용하는 POST 요청을 제외한 항상 기본적으로 200이다. 이는 핸들러에서 @HttpCode(…) 데코레이터를 사용해 쉽게 변경할 수 있다. |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
